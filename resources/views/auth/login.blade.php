...
<head>
    ...
    <title>مساعد الإنتاجية الذكي - تسجيل الدخول</title>
    ...
    <div class="max-w-md w-full bg-slate-900/80 backdrop-blur-xl border border-slate-800 rounded-3xl p-8 shadow-2xl text-right">
        ...
        <div class="text-center mb-6">
            <div class="w-12 h-12 bg-indigo-500/10 border border-indigo-500/30 rounded-2xl flex items-center justify-center mx-auto mb-3">
                <i class="ri-login-box-line text-2xl text-indigo-400"></i>
            </div>
            <h2 class="text-2xl font-extrabold text-white">مرحباً بك مجدداً!</h2>
            <p class="text-slate-400 text-xs mt-1">سجل دخولك لتتابع مهامك الذكية والتقويم اليومي</p>
        </div>
        ...
        <form action="{{ route('login') }}" method="POST" class="space-y-4">
            ...
            <div>
                <label class="text-xs font-bold text-slate-400 block mb-1">كلمة المرور</label>
                <div class="relative">
            ...
            <!-- خيار تذكرني المريح للمستخدم -->
            <div class="flex items-center justify-between py-1 select-none">
                <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="remember" class="rounded border-slate-700 bg-slate-800 text-indigo-600 focus:ring-indigo-500 h-4 w-4">
                    <span class="text-xs text-slate-400">تذكرني في هذا الجهاز</span>
                </label>
            </div>
            ...
            <button type="submit" 
                ...
                تسجيل الدخول الآمن <i class="ri-arrow-left-line mr-1"></i>
            </button>
        </form>

        <div class="text-center mt-6 border-t border-slate-800 pt-4">
            <span class="text-xs text-slate-400">ليس لديك حساب بعد؟</span>
            <a href="{{ route('register') }}" class="text-xs text-indigo-400 hover:text-indigo-300 font-bold mr-1">أنشئ حساباً مجانياً <i class="ri-arrow-left-line"></i></a>
        </div>
    </div>
